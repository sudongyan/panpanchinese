...
class Teacher extends Model {
	...
	public static function getList($p, $order) {
		...
		$list = Teacher::where($data) -> page($p, 100) -> order('create_time', $order) -> select();
		return $list;
	}
	
	//按id获取列表
	public static function getSomeList($idlist) {
		$data['state'] = 1;
		$list = Teacher::where('id','in',$idlist) -> select();
		return $list;
	}
	...
	public static function getSearchList($s, $p, $order) {
		...
		$list = Teacher::where($data) -> where('nickName', 'like', '%' . $s . '%') -> page($p, 100) -> order('create_time', $order) -> select();
		return $list;
	}
